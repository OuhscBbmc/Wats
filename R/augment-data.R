...
#' @usage augment_year_data_with_month_resolution( ds_linear, dateName )
#' augment_year_data_with_second_resolution( ds_linear, dateName )
#'
...
#' @param dateName The variable name in `ds_linear` containing the date or datetime value.
...
augment_year_data_with_month_resolution <- function( ds_linear, dateName ) {
  isMin <- NULL # avoid "Undefined global functions or variables"

  yearOfEvent <- lubridate::year(ds_linear[[dateName]])

  minYearOfEvent <- base::min(yearOfEvent)
  ds_linear$CycleTally <- (yearOfEvent - minYearOfEvent)
  monthsThroughTheYear <- lubridate::month(ds_linear[[dateName]]) - .5
  monthsInTheYear <- 12L
...
augment_year_data_with_second_resolution <- function( ds_linear, dateName ) {
  isMin <- NULL # avoid "Undefined global functions or variables"
  ...
  ds_linear$CycleTally <- (yearOfEvent - minYearOfEvent)
  secondsThroughTheYear <- base::as.integer(base::difftime(time1=ds_linear[[dateName]], firstOfYear, units="sec")) - .5
...
# ds_linear <- augment_year_data_with_second_resolution(ds_linear=ds_linear, dateName="Date")
